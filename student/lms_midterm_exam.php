...
$random_count = (int)($settings['midterm_random_count'] ?? 0);
...
function lms_best_midterm_attempt(PDO $pdo, int $uid, int $subject_id, int $pass_score): ?array {
    $st = $pdo->prepare("SELECT * FROM lms_student_midterm_exam WHERE student_uid=? AND subject_id=? ORDER BY score DESC, id ASC LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$attempts_exhausted) {
    ...
    if (!empty($shown_ids)) {
        ...
        $qs = $pdo->prepare("SELECT * FROM lms_midterm_questions WHERE subject_id=? AND id IN ($ph)");
    ...
    if ($result['is_final']) {
        $best = lms_best_midterm_attempt($pdo, $uid, $subject_id, $midterm_pass);
        if ($best) {
...
<div class="text-white px-5 pt-5 pb-6 shadow-xl" style="background:linear-gradient(135deg,#4F46E5,#3730A3)">
  <div class="flex items-center gap-3 mb-1">
    ...
    <div>
      ...
      <p class="text-indigo-200 text-xs font-bold"><?=htmlspecialchars($subject['subject_name'],ENT_QUOTES,'UTF-8')?> · ครั้งที่ <?=min($attempt_no,$max_att)?> / <?=$max_att?></p>
    </div>
  </div>
  <?php if (!$result): ?>
