...
$random_count = (int)($settings['final_random_count'] ?? 0);

// Check exam time window
$now_ts     = time();
$open_ts    = !empty($settings['final_open_at'])  ? strtotime($settings['final_open_at'])  : null;
$close_ts   = !empty($settings['final_close_at']) ? strtotime($settings['final_close_at']) : null;
$window_set = ($open_ts || $close_ts);
$in_window  = (!$open_ts || $now_ts >= $open_ts) && (!$close_ts || $now_ts <= $close_ts);

// Already passed?
$already_passed = $pdo->prepare("SELECT id FROM lms_student_final_exam WHERE student_uid=? AND subject_id=? AND passed=1 LIMIT 1");
...
// Attempt count
$stmt = $pdo->prepare("SELECT COUNT(*) FROM lms_student_final_exam WHERE student_uid=? AND subject_id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($shown_ids)) {
        ...
        $qs = $pdo->prepare("SELECT * FROM lms_final_questions WHERE subject_id=? AND id IN ($ph)");
        $qs->execute(array_merge([$subject_id], $shown_ids));
    } else {
        $qs = $pdo->prepare("SELECT * FROM lms_final_questions WHERE subject_id=? ORDER BY id");
        $qs->execute([$subject_id]);
    }
    $questions_mid = $qs->fetchAll();
    ...
    $passed = ($total_auto === 0 || $score >= $final_pass) ? 1 : 0;
    $tab_switch_count = max(0, (int)($_POST['tab_switch_count'] ?? 0));
    $pdo->prepare("INSERT INTO lms_student_final_exam (student_uid,subject_id,score,total,passed,attempt_no,tab_switch_count) VALUES (?,?,?,?,?,?,?)")
        ->execute([$uid,$subject_id,$score,$total_auto,$passed,$attempt_no,$tab_switch_count]);
    $exam_record_id = (int)$pdo->lastInsertId();
    $itemStmt = $pdo->prepare("INSERT INTO lms_exam_item_results (student_uid,subject_id,exam_type,exam_record_id,question_id,is_correct) VALUES (?,?,'final',?,?,?)");
    foreach ($answers as $a) {
        if (!$a['auto'] && (!empty($a['text']) || !empty($a['file_path']))) {
            $pdo->prepare("INSERT INTO lms_student_exam_answers (student_uid,subject_id,exam_type,exam_record_id,question_id,answer_text,file_path) VALUES (?,?,'final',?,?,?,?)")
...
$qs = $pdo->prepare("SELECT * FROM lms_final_questions WHERE subject_id=? ORDER BY id");
$qs->execute([$subject_id]); $all_questions = $qs->fetchAll();
...
<title>แบบทดสอบปลายภาค | โรงเรียนละลมวิทยา</title>
...
.choice-label:has(input:checked) { background: #D97706; color: white; border-color: #D97706; }
.choice-label:has(input:checked) .choice-letter { background: white; color: #D97706; }
</style>
</head>
<body class="min-h-screen bg-slate-50" style="padding-top:env(safe-area-inset-top)">

<div class="text-white px-5 pt-5 pb-6 shadow-xl" style="background:linear-gradient(135deg,#D97706,#92400E)">
  ...
  <div class="mt-4 bg-white/15 rounded-2xl px-4 py-3 flex items-center justify-between border border-white/20">
    <span class="text-sm font-bold"><?=$total_q?> ข้อ · ผ่านเกณฑ์ <?=$final_pass?> ข้อ</span>
...
<form method="POST" id="examForm" class="px-4 py-5 max-w-2xl mx-auto space-y-4 pb-24" enctype="multipart/form-data">
  ...
  <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <p class="text-sm font-bold text-slate-800 mb-1 leading-snug">
      <span class="text-amber-600 font-black mr-1"><?=$i+1?>.</span>
  ...
  <div class="max-w-2xl mx-auto flex items-center gap-4">
    <div class="flex-1">
      ...
      <div class="w-full bg-slate-100 rounded-full h-1.5">
        <div id="progressBar" class="h-1.5 rounded-full bg-amber-500 transition-all" style="width:0%"></div>
      </div>
    </div>
    <button type="button" onclick="submitExam()" class="px-6 py-2.5 bg-amber-600 text-white font-bold text-sm rounded-xl shadow-lg shadow-amber-200/50 active:scale-95 transition-transform">
      ส่งคำตอบ
    </button>
  </div>
</div>
...
function submitExam() {
  ...
  if (cnt < total) {
    Swal.fire({icon:'warning',title:'ยังไม่ครบ',text:`กรุณาตอบให้ครบทุกข้อ (ตอบแล้ว ${cnt}/${total} ข้อ)`,confirmButtonColor:'#D97706'});
    return;
  }
  Swal.fire({
