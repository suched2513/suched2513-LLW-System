...
$migrationName = '2026_05_02_000006_create_edoc_tables';
$migrationFile = __DIR__ . '/../database/migrations/' . $migrationName . '.php';

$steps = [
    'สร้างตาราง edoc_* (e-สารบรรณ)' => function(PDO $pdo) use ($migrationFile, $migrationName) {
        // ข้ามถ้าเคยรันแล้ว
        try {
            $done = (int)$pdo->query("SELECT COUNT(*) FROM _migrations WHERE migration = '$migrationName'")->fetchColumn();
            if ($done > 0) return 'already_done';
        } catch (Exception $e) { /* _migrations อาจยังไม่มี */ }

        $migration = require $migrationFile;
        $migration['up']($pdo);

        // บันทึกใน _migrations
        try {
...
foreach ($steps as $label => $fn) {
    try {
        $note = $fn($pdo);
        $results[] = ['ok' => true, 'label' => $label, 'note' => $note === 'already_done' ? '(เคยรันแล้ว)' : '✓ สร้างตารางใหม่'];
    } catch (Exception $e) {
...
?><!DOCTYPE html>
<html lang="th"><head><meta charset="UTF-8"><title>Migration — e-สารบรรณ</title>
...
<h2>Migration Results — ระบบ e-สารบรรณ</h2>
...
<?php endif; ?>
<p>
  <a href="/edocument/index.php">→ ไปที่ระบบ e-สารบรรณ</a>
  &nbsp;|&nbsp;
