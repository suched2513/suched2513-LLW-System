...
$migrationName = '2026_05_09_000001_fix_wfh_timelogs_fk_to_llw_users';
$migrationFile = __DIR__ . '/../database/migrations/' . $migrationName . '.php';

$steps = [
    'แก้ไข FK: wfh_timelogs.user_id → llw_users(user_id)' => function(PDO $pdo) use ($migrationFile, $migrationName) {
        try {
...
foreach ($steps as $label => $fn) {
    try {
        $note = $fn($pdo);
        $results[] = ['ok' => true, 'label' => $label, 'note' => $note === 'already_done' ? '(เคยรันแล้ว)' : '✓ แก้ไข FK สำเร็จ'];
    } catch (Exception $e) {
...
?><!DOCTYPE html>
<html lang="th"><head><meta charset="UTF-8"><title>Migration — WFH FK Fix</title>
<style>body{font-family:sans-serif;max-width:640px;margin:40px auto;padding:20px}
.ok{color:#16a34a}.err{color:#dc2626}.box{border:1px solid #e2e8f0;border-radius:8px;padding:16px;margin:8px 0}
.note{color:#64748b;font-size:.85em;margin-left:.5em}</style>
</head><body>
<h2>Migration — แก้ไข FK ระบบลงเวลา WFH</h2>
...
<?php else: ?>
<p class="err"><strong>❌ มีข้อผิดพลาด กรุณาตรวจสอบ</strong></p>
<?php endif; ?>
<p><a href="/user/dashboard.php">→ ทดสอบลงเวลา</a> &nbsp;|&nbsp; <a href="/admin/dashboard.php">← Admin Dashboard</a></p>
</body></html>
