...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['run'])) {
    ...
    foreach ($pending as $file) {
        ...
        try {
            ...
            if (is_array($migration) && isset($migration['up'])) {
                ...
                echo "<span class='err'>  ✘ Invalid migration format</span>\n";
            }
        } catch (Throwable $e) {
    ...
    echo '<pre>';
    if (empty($pending)) {
        echo "<span class='ok'>&#10003; ไม่มี pending migrations ทั้งหมด up-to-date แล้ว</span>\n";
    } else {
        echo "<span class='warn'>Pending migrations (" . count($pending) . " รายการ):</span>\n\n";
        foreach ($pending as $f) {
            echo "  • " . htmlspecialchars(basename($f)) . "\n";
        }
    }
    echo '</pre>';
    if (!empty($pending)) {
