...
try {
    ...
    foreach ($rows as &$r) {
        ...
        $r['usage_pct']  = $r['alloc_total'] > 0
            ? round(($r['used_total'] + $r['committed_total']) / $r['alloc_total'] * 100, 1) : 0;
    }
    unset($r);
} catch (Exception $e) {
    $rows = [];
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
  ...
  <div class="d-flex gap-2">
    <a href="<?= BASE_URL ?>/reports/export_excel.php?type=annual&fy=<?= $fy ?>" class="btn btn-sm btn-success">
      <i class="bi bi-file-earmark-excel me-1"></i>Excel
    </a>
    <a href="<?= BASE_URL ?>/reports/export_pdf.php?type=annual&fy=<?= $fy ?>" class="btn btn-sm btn-danger" target="_blank">
...
<div class="row g-3 mb-4">
  <div class="col-6 col-md-3">
    <div class="stat-card" style="background:linear-gradient(135deg,#1a56db,#3b82f6)">
      ...
      <div class="stat-label">งบจัดสรรทั้งหมด</div>
    </div>
  </div>
  <div class="col-6 col-md-3">
  ...
  <div class="col-6 col-md-3">
    <div class="stat-card" style="background:linear-gradient(135deg,#10b981,#34d399)">
      ...
      <div class="stat-label">คงเหลือ/คืนงบ</div>
    </div>
  </div>
</div>
    ...
    <table class="table table-hover mb-0">
      <thead>
        <tr>
          ...
          <th class="text-center">% ใช้แล้ว+ผูกพัน</th>
        </tr>
      </thead>
      <tbody>
      ...
      <tr>
        ...
        <td class="text-end text-warning"><?= $r['committed_total'] > 0 ? formatMoney($r['committed_total']) : '<span class="text-muted">-</span>' ?></td>
        <td class="text-end fw-semibold <?= $avail < 0 ? 'text-danger' : 'text-success' ?>"><?= formatMoney($avail) ?></td>
        <td class="text-center">
          <span class="badge bg-<?= $pct > 90 ? 'danger' : ($pct > 70 ? 'warning text-dark' : 'success') ?>">
            <?= number_format($pct, 1) ?>%
          </span>
        </td>
      </tr>
      ...
      <tr class="table-light fw-bold">
        ...
        <td class="text-center">
          <?php $overall = $totalAlloc > 0 ? round(($totalUsed + $totalCommitted) / $totalAlloc * 100, 1) : 0; ?>
          <span class="badge bg-<?= $overall > 90 ? 'danger' : ($overall > 70 ? 'warning text-dark' : 'success') ?>">
