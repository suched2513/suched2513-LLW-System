...
$stmtOverdue->execute([$fy]); $overdue = $stmtOverdue->fetchAll();
$fyAD = $fy - 543;
$stmtMonthly = $db->prepare("SELECT DATE_FORMAT(created_at,'%Y-%m') AS month, COUNT(*) AS cnt FROM project_requests WHERE YEAR(created_at)>=? GROUP BY month ORDER BY month DESC LIMIT 12");
$stmtMonthly->execute([$fyAD]); $monthly = $stmtMonthly->fetchAll();
