...
if (isset($_GET['export']) || isset($_GET['template'])) {
    $fn = isset($_GET['export']) ? 'lms_pre_exam_'.$unit_id.'_'.date('YmdHis').'.csv' : 'lms_pre_exam_template.csv';
    ...
    if (isset($_GET['export'])) {
        $st = $pdo->prepare("SELECT question_text,choice1,choice2,choice3,choice4,correct_answer,choice5 FROM lms_pre_questions WHERE unit_id=? AND question_type='choice' ORDER BY id");
        $st->execute([$unit_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'import' && !empty($_FILES['csv_file']['name'])) {
        ...
        while (($row = fgetcsv($h)) !== false) {
            ...
            if (count($row) >= 6 && trim($row[0]) && $ansNum >= 1 && $ansNum <= 5 && !($ansNum === 5 && $choice5 === '')) {
                $pdo->prepare("INSERT INTO lms_pre_questions (subject_id,unit_id,question_text,choice1,choice2,choice3,choice4,choice5,correct_answer) VALUES (?,?,?,?,?,?,?,?,?)")
                    ->execute([$subject_id,$unit_id,trim($row[0]),trim($row[1]),trim($row[2]),trim($row[3]),trim($row[4]),$choice5,$ansNum]);
                $cnt++;
            }
        }
        fclose($h);
        header('Location: pre_exam.php?unit_id='.$unit_id.'&msg='.urlencode("success:นำเข้าสำเร็จ $cnt ข้อ")); exit();
    }
    ...
    try {
        ...
        if ($action === 'add') {
            $pdo->prepare("INSERT INTO lms_pre_questions (subject_id,unit_id,question_text,question_type,question_img,choice1,choice2,choice3,choice4,choice5,correct_answer,options_json,correct_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)")
                ->execute([$subject_id,$unit_id,$qtext,$qtype,$qimg,$c[1],$c[2],$c[3],$c[4],$c[5],$ans,$options_json,$correct_json]);
            $msg = 'success:เพิ่มข้อสอบสำเร็จ';
        } elseif ($action === 'edit') {
            $pdo->prepare("UPDATE lms_pre_questions SET question_text=?,question_type=?,question_img=?,choice1=?,choice2=?,choice3=?,choice4=?,choice5=?,correct_answer=?,options_json=?,correct_json=? WHERE id=? AND unit_id=?")
                ->execute([$qtext,$qtype,$qimg,$c[1],$c[2],$c[3],$c[4],$c[5],$ans,$options_json,$correct_json,$id,$unit_id]);
            $msg = 'success:แก้ไขสำเร็จ';
        }
    } catch (Exception $e) { $msg = 'error:'.$e->getMessage(); }
    header('Location: pre_exam.php?unit_id='.$unit_id.'&msg='.urlencode($msg)); exit();
}
if (isset($_GET['action']) && $_GET['action'] === 'delete') {
    $pdo->prepare("DELETE FROM lms_pre_questions WHERE id=? AND unit_id=?")->execute([(int)$_GET['id'],$unit_id]);
    header('Location: pre_exam.php?unit_id='.$unit_id.'&msg='.urlencode('success:ลบสำเร็จ')); exit();
}
if (isset($_GET['msg'])) $msg = $_GET['msg'];

$st = $pdo->prepare("SELECT * FROM lms_pre_questions WHERE unit_id=? ORDER BY id");
$st->execute([$unit_id]);
...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
  <div class="flex items-center gap-3">
    ...
    <div>
      ...
      <p class="text-xs text-slate-400"><?=$total?> ข้อ · หน่วย: <?=htmlspecialchars($unit['unit_name'],ENT_QUOTES,'UTF-8')?> · <?=htmlspecialchars($subject['subject_name'] ?? '',ENT_QUOTES,'UTF-8')?></p>
    </div>
  </div>
  <div class="flex gap-2 flex-wrap">
    <a href="pre_exam.php?unit_id=<?=$unit_id?>&template=1" class="px-3 py-2 bg-slate-100 text-slate-600 text-xs font-bold rounded-xl hover:bg-slate-200 transition-all">
    ...
    <a href="pre_exam.php?unit_id=<?=$unit_id?>&export=1" class="px-3 py-2 bg-slate-500 text-white text-xs font-bold rounded-xl hover:bg-slate-600 transition-all">
      <i class="fas fa-file-export mr-1"></i> Export
    </a>
    <button onclick="openAddModal()" class="px-3 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all">
    ...
    <tbody class="divide-y divide-slate-50">
      ...
      <tr class="hover:bg-slate-50/50 transition-colors">
        ...
        <td class="px-5 py-3 text-center">
          <span class="px-2.5 py-1 bg-blue-50 text-blue-600 text-xs font-black rounded-full"><i class="<?=htmlspecialchars($meta['icon'],ENT_QUOTES,'UTF-8')?> mr-1"></i><?=htmlspecialchars($meta['label'],ENT_QUOTES,'UTF-8')?></span>
        </td>
        <td class="px-5 py-3">
          <div class="flex gap-2 justify-center">
            ...
            <button onclick="confirmDel('pre_exam.php?action=delete&id=<?=$q['id']?>&unit_id=<?=$unit_id?>')"
  ...
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
    ...
    <form method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
      ...
      <div>
        ...
        <select name="question_type" id="<?=$mid?>_qtype" onchange="renderQFields('<?=$mid?>')" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none" <?=$mid==='edit'?'disabled':''?>>
