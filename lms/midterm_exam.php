...
if (isset($_GET['export']) || isset($_GET['template'])) {
    $fn = isset($_GET['export']) ? 'lms_midterm_exam_'.$subject_id.'_'.date('YmdHis').'.csv' : 'lms_midterm_exam_template.csv';
    ...
    if (isset($_GET['export'])) {
        $st = $pdo->prepare("SELECT question_text,choice1,choice2,choice3,choice4,correct_answer,choice5 FROM lms_midterm_questions WHERE subject_id=? AND question_type='choice' ORDER BY id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'import' && !empty($_FILES['csv_file']['name'])) {
        ...
        while (($row = fgetcsv($h)) !== false) {
            ...
            if (count($row) >= 6 && trim($row[0]) && $ansNum >= 1 && $ansNum <= 5 && !($ansNum === 5 && $choice5 === '')) {
                $pdo->prepare("INSERT INTO lms_midterm_questions (subject_id,question_text,choice1,choice2,choice3,choice4,choice5,correct_answer) VALUES (?,?,?,?,?,?,?,?)")
                    ->execute([$subject_id,trim($row[0]),trim($row[1]),trim($row[2]),trim($row[3]),trim($row[4]),$choice5,$ansNum]);
                $cnt++;
            }
        }
        fclose($h);
        header('Location: midterm_exam.php?subject_id='.$subject_id.'&msg='.urlencode("success:นำเข้าสำเร็จ $cnt ข้อ")); exit();
    }
    ...
    try {
        ...
        if ($action === 'add') {
            $pdo->prepare("INSERT INTO lms_midterm_questions (subject_id,question_text,question_type,question_img,choice1,choice2,choice3,choice4,choice5,correct_answer,options_json,correct_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
                ->execute([$subject_id,$qtext,$qtype,$qimg,$c[1],$c[2],$c[3],$c[4],$c[5],$ans,$options_json,$correct_json]);
            $msg = 'success:เพิ่มข้อสอบสำเร็จ';
        } elseif ($action === 'edit') {
            $pdo->prepare("UPDATE lms_midterm_questions SET question_text=?,question_type=?,question_img=?,choice1=?,choice2=?,choice3=?,choice4=?,choice5=?,correct_answer=?,options_json=?,correct_json=? WHERE id=? AND subject_id=?")
                ->execute([$qtext,$qtype,$qimg,$c[1],$c[2],$c[3],$c[4],$c[5],$ans,$options_json,$correct_json,$id,$subject_id]);
            $msg = 'success:แก้ไขสำเร็จ';
        }
    } catch (Exception $e) { $msg = 'error:'.$e->getMessage(); }
    header('Location: midterm_exam.php?subject_id='.$subject_id.'&msg='.urlencode($msg)); exit();
}
if (isset($_GET['action']) && $_GET['action'] === 'delete') {
    $pdo->prepare("DELETE FROM lms_midterm_questions WHERE id=? AND subject_id=?")->execute([(int)$_GET['id'],$subject_id]);
    header('Location: midterm_exam.php?subject_id='.$subject_id.'&msg='.urlencode('success:ลบสำเร็จ')); exit();
}
if (isset($_GET['msg'])) $msg = $_GET['msg'];

$st = $pdo->prepare("SELECT * FROM lms_midterm_questions WHERE subject_id=? ORDER BY id");
$st->execute([$subject_id]);
$questions = $st->fetchAll();
$total = count($questions);

$pageTitle    = 'ข้อสอบกลางภาค';
...
<script>document.addEventListener('DOMContentLoaded',()=>{
  ...
  Swal.fire({icon:p[0]==='success'?'success':'error',title:p[0]==='success'?'สำเร็จ':'ผิดพลาด',text:p[1],confirmButtonColor:'#4F46E5',timer:p[0]==='success'?2000:undefined,showConfirmButton:p[0]!=='success'});
});</script>
<?php endif; ?>

<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
  <div class="flex items-center gap-3">
    <div class="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg bg-gradient-to-br from-indigo-500 to-indigo-700">
    ...
    <div>
      ...
      <h2 class="text-lg font-black text-slate-800">ข้อสอบกลางภาค</h2>
      <p class="text-xs text-slate-400"><?=$total?> ข้อ · <?=htmlspecialchars($subject['subject_name'],ENT_QUOTES,'UTF-8')?></p>
    </div>
  </div>
  <div class="flex gap-2 flex-wrap">
    <a href="midterm_exam.php?subject_id=<?=$subject_id?>&template=1" class="px-3 py-2 bg-slate-100 text-slate-600 text-xs font-bold rounded-xl hover:bg-slate-200 transition-all">
    ...
    <a href="midterm_exam.php?subject_id=<?=$subject_id?>&export=1" class="px-3 py-2 bg-slate-500 text-white text-xs font-bold rounded-xl hover:bg-slate-600 transition-all">
      <i class="fas fa-file-export mr-1"></i> Export
    </a>
    <button onclick="openAddModal()" class="px-3 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all">
    ...
    <tbody class="divide-y divide-slate-50">
      ...
      <tr class="hover:bg-slate-50/50 transition-colors">
        ...
        <td class="px-5 py-3 text-center">
          <span class="px-2.5 py-1 bg-indigo-50 text-indigo-600 text-xs font-black rounded-full"><i class="<?=htmlspecialchars($meta['icon'],ENT_QUOTES,'UTF-8')?> mr-1"></i><?=htmlspecialchars($meta['label'],ENT_QUOTES,'UTF-8')?></span>
        </td>
        <td class="px-5 py-3">
          <div class="flex gap-2 justify-center">
            ...
            <button onclick="confirmDel('midterm_exam.php?action=delete&id=<?=$q['id']?>&subject_id=<?=$subject_id?>')"
...
<?php foreach ([['add','เพิ่มข้อสอบกลางภาค'],['edit','แก้ไขข้อสอบ']] as [$mid,$mtitle]): ?>
<div id="<?=$mid?>Modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4" style="background:rgba(0,0,0,0.4)">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
      <h3 class="font-black text-slate-800"><i class="fas fa-clipboard-list mr-2 text-indigo-500"></i><?=$mtitle?></h3>
    ...
    <form method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
      ...
      <div>
        ...
        <select name="question_type" id="<?=$mid?>_qtype" onchange="renderQFields('<?=$mid?>')" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 outline-none" <?=$mid==='edit'?'disabled':''?>>
      ...
      <div>
        ...
        <textarea name="question_text" id="<?=$mid?>_qtext" rows="3" required class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 outline-none resize-none" placeholder="พิมพ์คำถาม..."></textarea>
      </div>
      ...
      <div class="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" class="px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all"><i class="fas fa-save mr-1"></i>บันทึก</button>
      </div>
    </form>
  </div>
</div>
