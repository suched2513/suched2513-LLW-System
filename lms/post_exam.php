...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'add') {
            $pdo->prepare("INSERT INTO lms_post_questions (subject_id,question_text,question_type,choice1,choice2,choice3,choice4,correct_answer) VALUES (?,?,?,?,?,?,?,?)")
                ->execute([$subject_id,$qtext,$qtype,$c[1],$c[2],$c[3],$c[4],$ans]);
            $msg = 'success:เพิ่มข้อสอบสำเร็จ';
        } elseif ($action === 'edit') {
            $pdo->prepare("UPDATE lms_post_questions SET question_text=?,question_type=?,choice1=?,choice2=?,choice3=?,choice4=?,correct_answer=? WHERE id=? AND subject_id=?")
                ->execute([$qtext,$qtype,$c[1],$c[2],$c[3],$c[4],$ans,$id,$subject_id]);
...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
  <div class="flex items-center gap-3">
    <div class="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg bg-gradient-to-br from-rose-500 to-red-600">
...
<div class="bg-white rounded-2xl shadow-xl shadow-slate-100/50 border border-slate-100 overflow-hidden">
  <table class="w-full text-sm">
    ...
    <tbody class="divide-y divide-slate-50">
      ...
      <tr><td colspan="5" class="py-16 text-center text-slate-300"><i class="fas fa-clipboard-check text-4xl mb-3 block opacity-30"></i>ยังไม่มีข้อสอบ</td></tr>
      <?php endif; ?>
...
<?php foreach ([['add','เพิ่มข้อสอบหลังเรียน'],['edit','แก้ไขข้อสอบ']] as [$mid,$mtitle]): ?>
<div id="<?=$mid?>Modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4" style="background:rgba(0,0,0,0.4)">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
      <h3 class="font-black text-slate-800"><i class="fas fa-clipboard-check mr-2 text-rose-500"></i><?=$mtitle?></h3>
    ...
    <form method="POST" class="p-6 space-y-4">
      ...
      <div>
        ...
        <div class="flex gap-4">
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="radio" name="question_type" value="choice" id="<?=$mid?>_type_choice" class="accent-rose-600" onchange="toggleChoices('<?=$mid?>',this.value)" checked>
            <span class="text-sm font-bold text-rose-700">ปรนัย (มีตัวเลือก)</span>
          </label>
          <label class="flex items-center gap-2 cursor-pointer">
      ...
      <div>
        ...
        <textarea name="question_text" id="<?=$mid?>_pqtext" rows="3" required class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-rose-400 outline-none resize-none" placeholder="พิมพ์คำถาม..."></textarea>
      </div>
      <div id="<?=$mid?>_choices_wrap">
        ...
        <div class="mb-3">
          ...
          <input type="text" name="choice<?=$n?>" id="<?=$mid?>_pc<?=$n?>" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-rose-400 outline-none">
        </div>
        <?php endfor; ?>
        <div>
          <label class="block text-xs font-black text-slate-500 mb-1">เฉลย</label>
          <select name="correct_answer" id="<?=$mid?>_pans" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-rose-400 outline-none">
      ...
      <div class="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" class="px-4 py-2 bg-rose-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-200 hover:bg-rose-700 transition-all"><i class="fas fa-save mr-1"></i>บันทึก</button>
      </div>
    </form>
  </div>
</div>
...
function openEdit(q){
  document.getElementById('peq_id').value=q.id;
  document.getElementById('edit_pqtext').value=q.question_text;
  ...
  for(let n=1;n<=4;n++) document.getElementById('edit_pc'+n).value=q['choice'+n]||'';
  document.getElementById('edit_pans').value=q.correct_answer;
  openModal('editModal');
}
