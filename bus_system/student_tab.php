...
<div id="sectionBusSystem" class="hidden mt-6 text-left fade-in">
    <div class="bg-gradient-to-br from-blue-50/50 to-indigo-50/50 rounded-2xl p-4 border border-blue-100/30">
        <h6 class="text-xs font-black text-blue-600 uppercase tracking-widest mb-3 flex items-center gap-2">
        ...
        <div id="busStatusCard" class="bg-white/80 rounded-2xl p-4 border border-blue-100 shadow-sm mb-4">
            <div class="flex justify-between items-start mb-3">
                <div>
                    <p class="text-xs font-black text-slate-400 uppercase">สายรถที่ใช้บริการ</p>
                    <h4 class="text-base font-black text-slate-800" id="busRouteName">-</h4>
                </div>
                <div id="busStatusBadge">
                    <span class="px-2 py-0.5 rounded-md bg-slate-100 text-slate-400 text-xs font-black uppercase">ยังไม่ลงทะเบียน</span>
                </div>
            </div>
            
            <div class="grid grid-cols-2 gap-3 mt-4">
                <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                    <p class="text-xs font-bold text-slate-400 uppercase">ยอดค้างชำระ</p>
                    <p class="text-lg font-black text-rose-500" id="busOwedAmount">฿0</p>
                </div>
                <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                    <p class="text-xs font-bold text-slate-400 uppercase">จ่ายแล้วรวม</p>
        ...
        <div class="mt-4 p-3 bg-white/40 rounded-xl border border-white/50">
            <h6 class="text-xs font-black text-slate-400 uppercase mb-2">ประวัติการชำระเงิน</h6>
            <div id="busPaymentHistory" class="space-y-2 max-h-[150px] overflow-y-auto pr-1 custom-scrollbar">
                <!-- Payment items -->
                <p class="text-xs text-slate-400 italic text-center py-2">ไม่มีประวัติการชำระเงิน</p>
            </div>
        </div>
    </div>
</div>
...
async function loadBusData(sid) {
    ...
    try {
        ...
        if (data.status === 'success' && data.data) {
            ...
            if (s === 'active') {
                statusBadge.innerHTML = '<span class="px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-600 text-xs font-black uppercase">กำลังใช้บริการ</span>';
                actions.innerHTML = `
                    <button onclick="requestBusCancel('${sid}')" class="w-full py-2.5 rounded-xl bg-white border border-rose-100 text-rose-600 text-xs font-bold hover:bg-rose-50 transition-all flex items-center justify-center gap-2">
                        <i class="bi bi-x-circle"></i> ขอยกเลิกใช้บริการ
                    </button>
                `;
            } else if (s === 'pending') {
                statusBadge.innerHTML = '<span class="px-2 py-0.5 rounded-md bg-amber-100 text-amber-600 text-xs font-black uppercase">รออนุมัติ</span>';
                actions.innerHTML = '<p class="text-xs text-amber-600 text-center font-bold">อยู่ระหว่างการตรวจสอบข้อมูล</p>';
            } else if (s === 'cancelled') {
                statusBadge.innerHTML = '<span class="px-2 py-0.5 rounded-md bg-rose-100 text-rose-600 text-xs font-black uppercase">ยกเลิกแล้ว</span>';
                actions.innerHTML = '<button onclick="openBusRegistrationModal()" class="w-full py-2.5 rounded-xl bg-blue-600 text-white text-xs font-bold shadow-lg shadow-blue-100 transition-all">ลงทะเบียนใหม่</button>';
            } else {
                actions.innerHTML = '<button onclick="openBusRegistrationModal()" class="w-full py-2.5 rounded-xl bg-blue-600 text-white text-xs font-bold shadow-lg shadow-blue-100 transition-all">ลงทะเบียนใช้บริการ</button>';
            }

            // Payment History
            if (b.payments && b.payments.length > 0) {
                historyList.innerHTML = b.payments.map(p => `
                    <div class="flex justify-between items-center p-2 bg-white/60 rounded-lg border border-white/50 text-xs">
                        <div>
                            <p class="font-bold text-slate-700">${new Date(p.payment_date).toLocaleDateString('th-TH')}</p>
                            <p class="text-xs text-slate-400">${p.note || 'ชำระค่าบริการ'}</p>
                        </div>
                        <p class="font-black text-emerald-600">฿${parseFloat(p.amount).toLocaleString()}</p>
                    </div>
                `).join('');
            } else {
                historyList.innerHTML = '<p class="text-xs text-slate-400 italic text-center py-2">ไม่มีประวัติการชำระเงิน</p>';
            }

            // Cancellation Check
            if (b.has_pending_cancel) {
                actions.innerHTML = '<div class="p-3 bg-amber-50 border border-amber-100 rounded-xl text-xs text-amber-700 font-bold text-center">ส่งคำขอยกเลิกแล้ว รอการตรวจสอบ</div>';
            }
