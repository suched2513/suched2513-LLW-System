...
try {
    ...
    // ตรวจสิทธิ์ att_teacher → อนุญาตให้เช็คชื่อเฉพาะห้องที่ตนเป็นที่ปรึกษา
    ...
    if ($classroom === 'all') {
        $stmt = $pdo->query("SELECT student_id, name, classroom FROM att_students WHERE academic_year = 2569 AND (status = 'active' OR status IS NULL) AND $filterSql ORDER BY classroom, student_id");
        $students = $stmt->fetchAll();
    } else {
        $stmt = $pdo->prepare("
            SELECT student_id, name, classroom
            FROM att_students
            WHERE classroom = ? AND academic_year = 2569 AND (status = 'active' OR status IS NULL) AND $filterSql
            ORDER BY student_id
        ");
