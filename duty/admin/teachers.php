...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'import_from_att') {
        ...
        if (!empty($ids)) {
            ...
            foreach ($attTeachers as $at) {
                ...
                if ($alreadyByAttId->fetchColumn()) continue;
    ...
    if ($action === 'add') {
        ...
        if ($fullName !== '') {
            $pdo->prepare("INSERT INTO duty_teachers (prefix, full_name, phone) VALUES (?, ?, ?)")
                ->execute([$prefix, $fullName, $phone]);
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                <thead class="table-light">
                    <tr>
                        <th>#</th>
                        <th class="text-center" style="width:70px">รูป</th>
                        <th>ชื่อ-สกุล</th>
                        <th>เบอร์โทร</th>
                    ...
                    <tr class="<?= $t['status'] === 'inactive' ? 'table-secondary opacity-50' : '' ?>">
                        ...
                        <td class="text-center">
                            <?php if (!empty($t['photo'])): ?>
                                <img src="/duty/api/photo.php?path=<?= urlencode($t['photo']) ?>"
                                     class="rounded-circle" style="width:44px;height:44px;object-fit:cover;border:2px solid #dee2e6;">
                            <?php else: ?>
                                <div class="rounded-circle bg-light d-flex align-items-center justify-content-center mx-auto"
                                     style="width:44px;height:44px;color:#adb5bd;font-size:18px;">
                                    <i class="fas fa-user"></i>
                                </div>
                            <?php endif; ?>
                            <button class="btn btn-outline-info btn-sm mt-1 py-0 px-1"
                                    title="อัปโหลดรูปโปรไฟล์"
                                    onclick="openPhotoUpload(<?= $t['id'] ?>, '<?= htmlspecialchars($t['prefix'] . $t['full_name'], ENT_QUOTES) ?>')">
...
<div class="modal fade" id="photoModal" tabindex="-1">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-camera me-2 text-info"></i>รูปโปรไฟล์</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body text-center">
                <p id="photoModalName" class="fw-bold text-primary mb-3"></p>
                <div id="photoPreview" class="mb-3" style="display:none;">
                    <img id="photoPreviewImg" class="rounded-circle"
                ...
                <input type="file" id="photoFile" accept="image/jpeg,image/png,image/webp"
                       class="form-control mb-3" onchange="previewPhoto(this)">
                <small class="text-muted d-block mb-3">รองรับ JPG, PNG, WEBP — ขนาดไม่เกิน 5 MB</small>
                <button id="photoUploadBtn" class="btn btn-primary w-100" onclick="uploadPhoto()">
...
let currentPhotoTeacherId = null;

function openPhotoUpload(teacherId, teacherName) {
    currentPhotoTeacherId = teacherId;
    document.getElementById('photoModalName').textContent = teacherName;
...
function uploadPhoto() {
    ...
    if (!file) {
        Swal.fire({icon:'warning', title:'กรุณาเลือกไฟล์รูปก่อน', confirmButtonColor:'#2563eb'});
        return;
    }
    const formData = new FormData();
    formData.append('photo', file);
    formData.append('teacher_id', currentPhotoTeacherId);
    ...
    fetch('/duty/api/upload_teacher_photo.php', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
        ...
        .catch(() => Swal.fire({icon:'error', title:'เกิดข้อผิดพลาด', text:'ไม่สามารถเชื่อมต่อได้', confirmButtonColor:'#2563eb'}))
        .finally(() => {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-upload me-1"></i> อัปโหลด';
        });
}
</script>
